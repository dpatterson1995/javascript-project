    ...
    if ( cartItems && productContainer){
        ...
        Object.values(cartItems).map(item => {
         ...
         <table><tbody><tr>
                <td>${item.name}</td>
                <td>${item.inCart}</td>
                <td id="itemCost$" class="alignright">$${item.inCart * item.price}</td>
                <td id="salesTax" class="alignright">${item.salesTax}</td>
                <td id="itemCostWTax" class="alignright">${!item.salesTax ? item.inCart * item.price : 1.06 * item.inCart * item.price}</td>
            </tr>
        </tbody></table>
         `   
        });
